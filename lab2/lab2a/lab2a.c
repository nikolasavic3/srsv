...
static Statistics stats = {0, 0, 0, 0};
...
static void obradi_zadatak(int sig, siginfo_t *info, void *context) {
    ...
    if (current_task.in_progress) {
        if (current_task.periods_used < 3 && !had_recent_extension()) {
            current_task.periods_used++;
            record_period(1);
            if (current_task.periods_used == 2) stats.used_two_periods++;
            else if (current_task.periods_used == 3) stats.used_three_periods++;
            printf("  [PROSIRENJE] Zadatak %d dobiva %d. period\n", current_task.task_id, current_task.periods_used);
...
void print_statistics() {
    ...
    printf("Koristilo tri perioda: %d\n", stats.used_three_periods);
    printf("\n=== STATISTIKA ULAZA ===\n");
