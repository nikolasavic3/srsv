...
void* input_simulator(void* arg) {
    ...
    while (simulation_running && t < 20000) {  //20s max
        ...
        if (states[i].state_changed == 1) {
            ...
            states[i].tmp_unprocessed_count++;
    ...
    return NULL;
                ...
                // if(states[i].tmp_unprocessed_count >= 3){
                //     // kriticni nacin rada
                    
                // }

int check_cond(int i){
    return states[i].state_changed;
}
int check_cond_critic(int i, int FLAG){
    return states[i].state_changed * states[i].tmp_unprocessed_count;
}

void* controller(void* arg) {
    while (simulation_running) {
        int FLAG = 0;
        int broj_obrada_u_ciklusu = 0;
        for (int i = 0; i < NUM_INPUTS; i++) {
            int uvjet = states[i].state_changed;
            pthread_mutex_lock(&mutex);
            if (FLAG ? check_cond_critic(i, FLAG) : check_cond(i)) {
                if(!FLAG && states[i].tmp_unprocessed_count >= 3){
                    printf("\nZapocinjem Kriticnu Obradu\n");
                    FLAG = 1;
                }
                ...
                // simulacija  procesa
                usleep(inputs[i].C * 1000 * 1);
                states[i].tmp_unprocessed_count -= 1;
                broj_obrada_u_ciklusu++;
        ...
        if(FLAG && (broj_obrada_u_ciklusu == 0)){
            printf("\nVracam se na normalnu obradu\n");
            FLAG = 0;
            continue;
        }
        usleep(1000000);
...
int main() {
    ...
    for (int i = 0; i < NUM_INPUTS; i++) {
        ...
        states[i].tmp_unprocessed_count = 0;
    }
