...
int check_cond_critic(int i){
    return states[i].state_changed * (states[i].tmp_unprocessed_count >= 3);
}
...
void* controller(void* arg) {
    while (simulation_running) {
        for (int i = 0; i < NUM_INPUTS; i++) {
            ...
            pthread_mutex_lock(&mutex);
            if (critic_mode ? check_cond_critic(i) : check_cond(i)) {                
...
void *check_critic(void* arg){
    while(simulation_running){
        ...
        for(int i=0; i<NUM_INPUTS;i++){
            if(states[i].tmp_unprocessed_count >= 3){
                if(!critic_mode){
                printf("\n KRITICNI NACIN RADA (zbog ulaze %d)\n", i);
                critic_mode = 1;
            }
            c++;
        }
...
int main() {
    ...
    pthread_create(&check, NULL, check_critic, NULL);
